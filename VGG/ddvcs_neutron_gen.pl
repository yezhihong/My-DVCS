...
$Energy = $ARGV[0]; # 11 or 8.8 GeV
$TargetPol = $ARGV[1]; #1->Tx, 2->Ty, 3->L
$TP="X";
if ($TargetPol == 1){
	$TP = "Tx";
}
...
$DATA_DIR = "$WORK_DIR/DDVCS_Neutron_$E0";
#########################################################################3
...
#		for ($it=0;$it<20;$it++){#upto <62 before
	    ...
	    while ($t<=$t_Max){
			...
			$filename = "$WORK_DIR/test/E$E0-$TP-Q2-$iQ2-xb-$ixb-t-$it";
...
#########################################################################3
				...
				$script_name = "$WORK_DIR/test/jsub_E$E0-$TP-Q2-$iQ2-xb-$ixb-t-$it";#_$ARGV[0]_$ARGV[1]_$ARGV[2]";
...
#			print OUT_FILE "MAIL: $mail_to\@jlab.org\n";
				print OUT_FILE "COMMAND: cat $WORK_DIR/test/E$E0-$TP-Q2-$iQ2-xb-$ixb-t-$it | $VGG_CMD\n";
