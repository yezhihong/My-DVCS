...
int main(int argc, char *argv[])
{
	...
	TGenSoLIDPi0 *gEv=new TGenSoLIDPi0(Ebeam, Target_Type, 0, 0);//last two parameters unused
	...
	//	gEv->GenFermiIni();//To use if fFermi is set
		...
		if(gEv->ComputeElectron()){//Compute the scattered electron 4-vector
			...
			if(gEv->HitsSoLID(gEv->GetScatteredElectron())){//Check hor spectro accep.
				...
				if(gEv->HitsSoLIDCalo(gEv->GetFinalPhoton1())||gEv->HitsSoLIDCalo(gEv->GetFinalPhoton2())){//Checks calo acceptance
