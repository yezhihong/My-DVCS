...
void plot_bin(const TString& TPol,const TString& energy,const int Q2_flag, const int xb_flag, const int t_flag){
	...
	double y_pos = 0.30;
	titlename.Form("#scale[0.7]{(#%d) %2.1f < Q^{2} < %2.1f}",Q2_flag,Q2_min,Q2_max);
	t1->DrawLatex(0.35,y_pos,titlename);
	titlename.Form("#scale[0.7]{(#%d) %3.2f < xb < %3.2f}",xb_flag,xb_min,xb_max);
	t1->DrawLatex(0.35,y_pos-0.06,titlename);
	titlename.Form("#scale[0.7]{(#%d) %3.2f < t < %3.2f}",t_flag,t_min,t_max);
