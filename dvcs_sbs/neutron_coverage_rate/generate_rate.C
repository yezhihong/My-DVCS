...
void generate_rato(TString particle,int energy_flag){
	...
	for(Long64_t i=0;i<N_entries;i++){
		...
		if(Q2>1&&phi>=0.0&&phi<360&&W>2.0 && Sigma_L >1e-25 && gTheta>0.0){//any additional cuts should be added in here
			...
			if(eP<3.5||eTheta<16.0||eTheta>24)//GeV,CLEO
...
//				pho_large_acceptance=0.0; 
	...
	/*Print&Save{{{*/
	ofstream outf(Form("%s_%d_he3_rate_q2gt1.dat", particle.Data(), energy_flag));
	...
	outf<<"photon rate_forward: "<<pho_rate_forward<<endl;
	outf<<"photon rate_large: "<<pho_rate_large<<endl;
	...
	cout<<"eletron rate_forward: "<<ele_rate_forward<<endl;
	cout<<"eletron rate_large: "<<ele_rate_large<<endl;
	cout<<"photon rate_forward: "<<pho_rate_forward<<endl;
	cout<<"photon rate_large: "<<pho_rate_large<<endl;
