...
int makebin(TString target, TString energy, int Q2_flag, int x_flag){
	...
	const double dilute_factor = 0.9;
	const double target_polarization = 0.6; //60% polarization
	const double beam_polarization = 1.0; //60% polarization
	...
	//Sigma
	TH1F *h1PP = new TH1F("h1PP","h1PP (log10)",1000,-12.0,6.);
	TH1F *h1PM = new TH1F("h1PM","h1PM (log10)",1000,-12.0,6.);
	TH1F *h1MP = new TH1F("h1MP","h1MP (log10)",1000,-12.0,6.);
	TH1F *h1MM = new TH1F("h1MM","h1MM (log10)",1000,-12.0,6.);
	TH1F *h1XS = new TH1F("h1XS","h1XS (log10)",1000,-12.0,6.);
	
	TH1F *h1BHp = new TH1F("h1BHp","h1BHp (log10)",1000,-12.0,6.);
	TH1F *h1BHm = new TH1F("h1BHm","h1BHm (log10)",1000,-12.0,6.);
	/*}}}*/
	...
	if(Q2_flag > Q2_bin-1) return -4;
	double Q2min = Q2_cut[Q2_flag];
	double Q2max = Q2_cut[Q2_flag+1];
	if(x_flag > x_bin-1) return -5;
	double xmin = x_cut[x_flag];
	double xmax = x_cut[x_flag+1];
	...
	for (Int_t i=0;i<tbin;i++){
		...
		prefix = Form("./new_database_%s/",energy.Data());
		...
		filename = Form("BH_%s_%d_%d_%d.dat",target.Data(),Q2_flag,x_flag, i);
		new_filename = prefix + filename;
		ofstream outf_BH(new_filename);
		...
		for (Int_t j=0;j<phi_bin;j++){
			...
			cut1="(weight*(XS_BHp+XS_BHm)/2.)";
			cut2.Form("(Q2>=%f&&Q2<%f&&x>=%f&&x<%f&&phi>=%f&&phi<%f&&t>=%f&&t<%f&&W>2.)",
					   Q2min, Q2max, xmin,xmax,phimin,phimax,tmin,tmax);
				cut2 = cut1 + "*" + cut2;
			cut = (TCut) (cut2);
			...
			T->Project("h1t","t",cut);
			T->Project("h1BHp","log10(XS_BHp)",cut);
			T->Project("h1BHm","log10(XS_BHm)",cut);

			dilution = dilute_factor;//fix the value for now
			N_out = h1phi->GetSum()*det_eff;
			//N_out = h1phi->GetSum()*(pow(target_factor * dilution,2) * det_eff);
			if(N_out>10)
				Astat = 1./sqrt(N_out);
			else
				Astat = -1.0;

			outf_BH<<Form("%4d %4d %10.4f %10.4f %10.4f %10.4f %10.4f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e",
					i,j,
					h1Q2->GetMean(),
					h1x->GetMean(),
					h1t->GetMean(),
					...
					Astat,
					N_out,
					pow(10., h1BHp->GetMean()),	
					pow(10., h1BHm->GetMean()),
					0.,0.,0.,0.,0.,0.)	
				<<endl;
			...
			cut1="(weight*Sigma_L*1.0)";
			cut2.Form("(Q2>=%f&&Q2<%f&&x>=%f&&x<%f&&phi>=%f&&phi<%f&&t>=%f&&t<%f&&W>2.)",
					   Q2min, Q2max, xmin,xmax,phimin,phimax,tmin,tmax);
			cut2 = cut1 + "*" + cut2;
			cut = (TCut) (cut2);

			//cerr<<"---- Cut = "<<cut2.Data()<<endl<<endl;

			T->Project("h1Q2","Q2",cut);
			T->Project("h1x","x",cut);
			T->Project("h1phi","phi",cut);
			T->Project("h1t","t",cut);
			T->Project("h1TSA","TSA_L/Sigma_L/1.0",cut);
			T->Project("h1BSA","BSA_L/Sigma_L/1.0",cut);
			T->Project("h1DSA","DSA_L/Sigma_L/1.0",cut);

			T->Project("h1PP","log10(SigmaPP_L)",cut);
			T->Project("h1PM","log10(SigmaPM_L)",cut);
			T->Project("h1MP","log10(SigmaMP_L)",cut);
			T->Project("h1MM","log10(SigmaMM_L)",cut);
			T->Project("h1XS","log10(Sigma_L)",cut);

			//c1->cd(1);
			//h1phi->Draw();
			//c1->Print(Form("Phi_Q2_%d_x_%d_t_%d_phi_%d.png", Q2_flag, x_flag, i,j));
			...
			outf_L<<Form("%4d %4d %10.4f %10.4f %10.4f %10.4f %10.4f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e",
					i,j,
					h1Q2->GetMean(),
					h1x->GetMean(),
					h1t->GetMean(),
			...
			T->Project("h1phi","phi",cut);
			T->Project("h1t","t",cut);
			T->Project("h1TSA","TSA_Tx/Sigma_Tx/1.0",cut);
			T->Project("h1BSA","BSA_Tx/Sigma_Tx/1.0",cut);
			T->Project("h1DSA","DSA_Tx/Sigma_Tx/1.0",cut);

			T->Project("h1PP","log10(SigmaPP_Tx)",cut);
			T->Project("h1PM","log10(SigmaPM_Tx)",cut);
			T->Project("h1MP","log10(SigmaMP_Tx)",cut);
			T->Project("h1MM","log10(SigmaMM_Tx)",cut);
			T->Project("h1XS","log10(Sigma_Tx)",cut);
			...
			outf_Tx<<Form("%4d %4d %10.4f %10.4f %10.4f %10.4f %10.4f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e",
					i,j,
					h1Q2->GetMean(),
					h1x->GetMean(),
					h1t->GetMean(),
			...
			T->Project("h1phi","phi",cut);
			T->Project("h1t","t",cut);
			T->Project("h1TSA","TSA_Ty/Sigma_Ty/1.0",cut);
			T->Project("h1BSA","BSA_Ty/Sigma_Ty/1.0",cut);
			T->Project("h1DSA","DSA_Ty/Sigma_Ty/1.0",cut);

			T->Project("h1PP","log10(SigmaPP_Ty)",cut);
			T->Project("h1PM","log10(SigmaPM_Ty)",cut);
			T->Project("h1MP","log10(SigmaMP_Ty)",cut);
			T->Project("h1MM","log10(SigmaMM_Ty)",cut);
			T->Project("h1XS","log10(Sigma_Ty)",cut);
			...
			outf_Ty<<Form("%4d %4d %10.4f %10.4f %10.4f %10.4f %10.4f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e",
					i,j,
					h1Q2->GetMean(),
					h1x->GetMean(),
					h1t->GetMean(),
		...
		outf_L.close(); outf_Tx.close(); outf_Ty.close(); outf_BH.close();
	}
	/*}}}*/
