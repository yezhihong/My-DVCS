...
{
	cout << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" << endl;
	cout << "&&&& SBS-DVCS Generator &&&&" << endl;
	cout << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" << endl;
	if(argc!=4) {
		cout << "Usage: GenDVCS <Ebeam(GeV)> <Target(0=P, 1=N)> <Total_Event>" << endl;
		return -1;
	}
	...
	TGenDVCS *gEv=new TGenDVCS(Ebeam, Target_Type, 0, 0);//last two parameters unused
    ...
    const Double_t SBS_Theta_Min = 0.0, SBS_Theta_Max = 26;
    const Double_t SBS_Phi_Min = 0.0, SBS_Phi_Max = 360;
	...
	const Double_t TDIS_Theta_Min = 0.0, TDIS_Theta_Max = 80;
	...
	const Double_t ECal_Theta_Min = 0.0, ECal_Theta_Max = 80;
    const Double_t ECal_Phi_Min = 0.0, ECal_Phi_Max = 360;
	...
	//	gEv->GenFermiIni();//To use if fFermi is set
        ...
        if(gEv->ComputeElectron()){//Compute the scattered electron 4-vector
            ...
            if(gEv->HitsSBS(gEv->GetScatteredElectron())){//Check hor spectro accep.
                gEv->ComputeDVCS();//Compute the gamma*n->gamma n collision
                gEv->ApplyAcc(-1, SBS_Theta_Min, SBS_Theta_Max );//Rotates all final vectors around the beam axis
                if(gEv->HitsTDIS(gEv->GetFinalProton())){//Checks TDIS acceptance
                    if(gEv->HitsECal(gEv->GetFinalPhoton())){//Checks calo acceptance
	...
	gEv->DumpRootFile(Form("SBS_DVCS_%s_%sGeV.root",Target_Name.Data(), Energy.Data()),nev,j);
