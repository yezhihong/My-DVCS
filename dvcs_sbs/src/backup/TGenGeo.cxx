...
{
  // Sets the angles of the spectro and calorimeter (+PRD), the central 
...
//_____________________________________________________________________________
 void TGenGeo::SetPRDAcceptance(Double_t PolAccMax, Double_t PolAccMin, Double_t AzimAccMax, Double_t AzimAccMin)
{
  // Sets proton array polar (max and min) and azimuthal (max and min)
  // acceptances

  fPRDPolarAccMax=PolAccMax;
  fPRDPolarAccMin=PolAccMin;
  fPRDAzimAccMax=AzimAccMax;
  fPRDAzimAccMin=AzimAccMin;
 ...
 void TGenGeo::SetDefaultAcceptances(void)
{
  ...
  SetCaloAcceptance(250.,250.,300.,300.);  // calorimeter is not centered
                                           // in final version
  SetPRDAcceptance(0.66323,0.31416,5.4978,0.78540);
...
//_____________________________________________________________________________
 Bool_t TGenGeo::HitsPRD(TLorentzVector* p) 
{ 
  ...
  if(vp.Angle(Oz)>fPRDPolarAccMin && vp.Angle(Oz)<fPRDPolarAccMax && (p->Px()<0.
  || (p->Px()>0. && (p->Py()>(p->Perp())*TMath::Sin(fPRDAzimAccMin)
  ||                 p->Py()<(p->Perp())*TMath::Sin(fPRDAzimAccMax))))){
 ...
 void TGenGeo::Print(char* opt)
{
  ...
  cout<<"Proton Recoil Dectector Acceptance : "<<fPRDPolarAccMax*180./TMath::Pi()<<" deg pol max "<<fPRDPolarAccMin*180./TMath::Pi()<<" deg pol min "<<fPRDAzimAccMax*180./TMath::Pi()<<" deg azi max "<<fPRDAzimAccMin*180./TMath::Pi()<<" deg azi min"<<endl;
  cout<<"================================================================="<<endl;
}
