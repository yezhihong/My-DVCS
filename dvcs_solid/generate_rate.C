...
void generate_rato(TString particle,int energy_flag){
	...
	for(Long64_t i=0;i<N_entries;i++){
		...
		if(phi*DEG>2.0&&phi*DEG<358&&W>2.0){//any additional cuts should be added in here
...
//				pho_large_acceptance=0.0; 
			...
			double event_weight=(XS_P+XS_M)*PSF/N_gen*nBcm2*Lumi ;   //in Hz
			...
			hf_phi->Fill(phi*DEG,event_weight*total_ele_forward_acceptance);
			hl_phi->Fill(phi*DEG,event_weight*total_ele_large_acceptance);
			h_phi->Fill( phi*DEG,event_weight*(total_acceptance));
			/*}}}*/
		}
